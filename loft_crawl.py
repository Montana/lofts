...
class CraigSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for post in posts:
            ...
            l.add_xpath('url', './/a[@class="result-title hdrlnk"]/@href',
            ...
            l.add_xpath('neighborhood',
                        './/span[@class="result-hood"]',
                        re='\((.*)\)')

            post = l.load_item()

            posted_ts = dt.datetime.strptime(post['posted_ts'],
                                             '%Y-%m-%d %H:%M')

            # stop if we've reached posts older than the cutoff
            if (dt.datetime.now() - posted_ts).days > MAX_POST_AGE_DAYS:
                raise CloseSpider('reached post age cutoff ({} days)'.format(
                    MAX_POST_AGE_DAYS))
    ...
    def parse_details(self, response):
        ...
        l.add_xpath(
            'available_date',
            '//span[@class="housing_movein_now property_date"]/@data-date')
        l.add_xpath('body', 'string(//section[@id="postingbody"])')

        for tag in [
                'cats are OK - purrr', 'dogs are OK - wooof', 'street parking',
                'off-street parking'
                'carport', 'attached garage', 'detached garage', 'no smoking',
                'laundry on site', 'laundry in bldg', 'w/d in unit',
                'apartment', 'condo', 'cottage/cabin', 'duplex', 'flat',
                'house', 'in-law', 'loft'
                'townhouse'
        ]:
